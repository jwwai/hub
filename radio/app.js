...
(() => {
  ...
  /* ---------------- Google Drive sync (shared user.db) ---------------- */
  // Syncs favourite stations to the SAME "user.db" file on Google Drive
  // that Live TV syncs its favourites to (same file, two top-level keys:
  // "radio" and "tv" — each app only ever reads/writes its own key, so
  // running both apps against one file never clobbers the other's data).
  //
  // Uses Google Identity Services' token-client flow (no backend needed —
  // this is a fully client-side OAuth pattern Google explicitly supports
  // for static/SPA apps). Because the target is a specific pre-existing
  // file the person already owns/shared (not one this app creates itself),
  // this needs the broader `drive` scope rather than the narrower
  // `drive.file`/`drive.appdata` scopes — those can only reach files the
  // app itself created or that were handed to it through Drive's picker,
  // neither of which fits "sync to this file I already have".
  //
  // Entirely inert (every function below no-ops immediately) unless
  // DRIVE_CLIENT_ID and DRIVE_FILE_ID are both set.
  const DRIVE_CLIENT_ID = '787332701448-tb0kq3hhk5s9vl0b15f6gde38uq7o3v1.apps.googleusercontent.com';
  const DRIVE_FILE_ID = '1LVoOzPspMPz_BL9Q6u06VhzhZH6Vf3jO'; // the "user.db" file — https://drive.google.com/file/d/1LVoOzPspMPz_BL9Q6u06VhzhZH6Vf3jO/view
  const DRIVE_SCOPE = 'https://www.googleapis.com/auth/drive';
  const DRIVE_DB_KEY = 'radio';
  ...
  function driveModalEls() {
    return {
      backdrop: $('driveSyncBackdrop'),
      status: $('driveSyncStatusText'),
      actionBtn: $('driveSyncActionBtn'),
      closeBtn: $('driveSyncClose'),
    };
  }

  // The sync UI lives in a standalone modal, opened from the Live Hub
  // shell's top bar (right next to the reload button) rather than from a
  // button inside this app — the shell posts a message into this iframe,
  // which is more robust than the shell reaching into this document's
  // DOM directly and works the same regardless of framing.
  function openDriveSyncModal() {
    const { backdrop } = driveModalEls();
  ...
  window.addEventListener('message', (e) => {
    if (e.data && e.data.type === 'liveHub:openSync') openDriveSyncModal();
  });

  function setDriveStatus(text, kind) {
    const { status, actionBtn } = driveModalEls();
    if (status) {
  ...
  function initGoogleDriveSync() {
    ...
    driveTokenClient = google.accounts.oauth2.initTokenClient({
      ...
      scope: DRIVE_SCOPE,
      callback: async (resp) => {
        if (resp.error) { setDriveStatus('Sign-in failed or was cancelled.', 'is-error'); return; }
    ...
    if (actionBtn) actionBtn.addEventListener('click', () => {
      if (driveAccessToken) {
        ...
        // grant itself (that's managed from the person's Google Account
        // permissions page, same as any other connected app).
  ...
  async function driveApiFetch(url, options) {
    const res = await fetch(url, {
      ...options,
      headers: { ...(options && options.headers ? options.headers : {}), Authorization: `Bearer ${driveAccessToken}` },
    });
    if (res.status === 401) {
      driveAccessToken = null;
      setDriveStatus('Session expired — sign in again', 'is-error');
      throw new Error('Drive session expired');
    }
    return res;
  }

  // Fetches the whole shared user.db JSON object (both apps' keys), never
  // just this app's slice — needed so pushes can preserve the other app's
  // data instead of overwriting the whole file with only "radio".
  async function pullUserDb() {
    const res = await driveApiFetch(`https://www.googleapis.com/drive/v3/files/${DRIVE_FILE_ID}?alt=media`);
    if (res.status === 404) throw new Error('user.db file not found — check the file ID and that it\u2019s shared with this account.');
    if (!res.ok) throw new Error(`Drive returned HTTP ${res.status}`);
    const text = await res.text();
    if (!text.trim()) return {};
    try { return JSON.parse(text); } catch { return {}; } // file exists but isn't valid JSON yet — start fresh rather than fail
  }

  async function pushUserDb(db) {
  ...
  // Full two-way sync: pull the shared file, merge Drive's "radio" entries
  // into what's local (Drive wins per-station on any key present in both —
  // there's no per-entry timestamp for real conflict resolution), save
  // locally, then push the merged result back — preserving whatever the
  // TV app has under "tv" untouched.
  async function syncFavoritesWithDrive() {
    if (driveSyncing || !driveAccessToken) return;
    driveSyncing = true;
    try {
      const db = await pullUserDb();
      const remoteFavs = (db && typeof db === 'object' && db[DRIVE_DB_KEY]) || {};
      Object.values(remoteFavs).forEach((station) => { if (station && station.url) state.favorites.set(station.url, station); });
      try { localStorage.setItem(LS_FAVS, JSON.stringify([...state.favorites.values()])); } catch { /* ignore */ }
      el.favCount.textContent = state.favorites.size;
      refreshFavVisuals();
      if (state.currentTab === 'FAV') renderActiveList();

      db[DRIVE_DB_KEY] = Object.fromEntries([...state.favorites.entries()]);
      await pushUserDb(db);
      setDriveStatus(`Synced just now (${state.favorites.size} favourites)`, 'is-ok');
    } catch (e) {
      console.warn('Drive favourites sync failed:', e);
      if (driveAccessToken) setDriveStatus(e.message || 'Sync failed — will retry on next change', 'is-error');
    } finally {
      driveSyncing = false;
    }
  }

  // Debounced push for ordinary favourite/unfavourite toggles during an
  // already-signed-in session — batches rapid changes into one upload,
  // and is a no-op whenever nobody's signed in (safe to call unconditionally
  // from saveFavorites).
  function scheduleDrivePush() {
    ...
    drivePushTimer = setTimeout(async () => {
      try {
        ...
        console.warn('Drive favourites push failed:', e);
        setDriveStatus(e.message || 'Sync failed — will retry on next change', 'is-error');
      }
    }, 1500);
  }
  ...
  (function init() {
    ...
    updateVolumeUI();
    loadSummary();
    initGoogleDriveSync(); // no-op unless DRIVE_CLIENT_ID / DRIVE_FILE_ID are set
  })();
})();
